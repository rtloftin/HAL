...
class Agent:
    ...
    def __init__(self, sensor, data, graph, session, **kwargs):
        ...
        with graph.as_default():
            ...
            visible = tf.Variable(dtype=tf.bool, trainable=False, use_resource=True)
            occupied = tf.Variable(dtype=tf.bool, trainable=False, use_resource=True)

            visible = tf.assign(visible, tf.not_equal(occupancy, Occupancy.UNKNOWN))
            occupied = tf.assign(occupied, tf.equal(occupancy, Occupancy.OCCUPIED))

            self._occupancy_update = tf.group(visible, occupied)
            ...
            probabilities = tf.where(visibility, tf.where(occupancy, 1., 0.), probabilities)
